...
VgeImgui::VgeImgui(Window& window, VgeDevice& device, Renderer& renderer, uint32_t imageCount,
                   std::unique_ptr<Scene>* scenePtr, VkDescriptorSetLayout globalSetLayout,
                   Input* input)
    : vgeDevice{device},
    ...
    if (!ImGui_ImplVulkan_Init(&init_info, vgeRenderer.getSwapChainRenderPass())) {
...
void VgeImgui::beginDockspace() {
    ...
    if (io.ConfigFlags & ImGuiConfigFlags_DockingEnable) {
        ...
        if (first_time) {
            ...
            auto dock_id_right = ImGui::DockBuilderSplitNode(dockspace_id, ImGuiDir_Right, 0.2f,
...
void VgeImgui::renderSceneSelector() {
    ...
    if (ImGui::Button("Load Galaxy Scene")) {
        ...
        if (!*currentScenePtr) {
            (*currentScenePtr) =
                std::make_unique<GalaxyScene>(vgeDevice, vgeRenderer, globalSetLayout);
        }
    }
