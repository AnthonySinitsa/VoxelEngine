...
VulkanApplication::VulkanApplication() {
  ...
  currentScene = std::unique_ptr<Scene>(new GalaxyScene(
      vgeDevice, vgeRenderer, globalSetLayout->getDescriptorSetLayout()));

  // Initialize ImGui
  vgeImgui = std::make_unique<VgeImgui>(
      vgeWindow, vgeDevice, vgeRenderer, VgeSwapChain::MAX_FRAMES_IN_FLIGHT,
      &currentScene, globalSetLayout->getDescriptorSetLayout(), &input);
}
...
void VulkanApplication::run() {
  ...
  for (int i = 0; i < uboBuffers.size(); i++) {
    uboBuffers[i] = std::make_unique<VgeBuffer>(
        vgeDevice, sizeof(GlobalUbo), 1, VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT,
  ...
  PointLightSystem pointLightSystem{vgeDevice,
...
void VulkanApplication::loadGameObjects() {
  ...
  auto pointLight =
      GameObject::makePointLight(1.0f,                       // intensity
                                 0.1f,                       // radius
