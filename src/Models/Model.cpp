...
namespace std {
template <> struct hash<vge::Model::Vertex> {
  size_t operator()(vge::Model::Vertex const &vertex) const {
    size_t seed = 0;
    vge::hashCombine(seed, vertex.position, vertex.color, vertex.normal,
                     vertex.uv);
    return seed;
  }
};
...
namespace vge {
Model::Model(VgeDevice &device, const Model::Builder &builder)
    : vgeDevice{device} {
...
std::unique_ptr<Model> Model::createModelFromFile(VgeDevice &device,
...
void Model::createVertexBuffer(const std::vector<Vertex> &vertices) {
  ...
  vertexBuffer = std::make_unique<VgeBuffer>(
      vgeDevice, vertexSize, vertexCount,
      VK_BUFFER_USAGE_VERTEX_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT,
      VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);

  vgeDevice.copyBuffer(stagingBuffer.getBuffer(), vertexBuffer->getBuffer(),
                       bufferSize);
}
...
void Model::createIndexBuffer(const std::vector<uint32_t> &indices) {
  ...
  indexBuffer = std::make_unique<VgeBuffer>(
      vgeDevice, indexSize, indexCount,
      VK_BUFFER_USAGE_INDEX_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT,
      VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);

  vgeDevice.copyBuffer(stagingBuffer.getBuffer(), indexBuffer->getBuffer(),
                       bufferSize);
}
...
void Model::draw(VkCommandBuffer commandBuffer) {
  if (hasIndexBuffer) {
    ...
    // This command draws geometry directly from the vertex buffer, without
    // using index buffer
...
void Model::Builder::loadModel(const std::string &filepath) {
  ...
  std::string warnAndErr;

  std::string baseDir = filepath.substr(0, filepath.find_last_of('/') + 1);

  if (!tinyobj::LoadObj(&attrib, &shapes, &tinyMaterials, &warnAndErr,
                        filepath.c_str(), baseDir.c_str())) {
    throw std::runtime_error(warnAndErr);
  }
  ...
  for (const auto &shape : shapes) {
    ...
    for (size_t i = 0; i < shape.mesh.indices.size();
         i += 3) { // Since we have triangles
      int materialId =
          shape.mesh.material_ids.empty() ? 0 : shape.mesh.material_ids[i / 3];
