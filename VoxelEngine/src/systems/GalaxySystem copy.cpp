...
//         stars.resize(GalaxyParameters::NUM_STARS);
//         for (int i = 0; i < GalaxyParameters::NUM_STARS; i++) {
//             float angle = static_cast<float>(i) * 0.1f;
//             float radius = (static_cast<float>(i) / GalaxyParameters::NUM_STARS) * 20.0f;

//             stars[i].position = glm::vec2(
//                 radius * cos(angle),
//                 radius * sin(angle)
//             );

//             // Vary colors based on distance from center
//             float temperature = 1.0f - (radius / 20.0f);  // Hotter in center
//             stars[i].color = glm::vec3(
//                 temperature,  // More red for hotter stars
//                 temperature * 0.8f,  // Less green for cooler stars
//                 temperature * 0.6f   // Even less blue for coolest stars
//             );
//             stars[i].normal = glm::vec3(0.0f, 0.0f, 1.0f);
//             stars[i].uv = glm::vec2(0.0f, 0.0f);
//         }
...
//         uint32_t numWorkgroups = (GalaxyParameters::NUM_STARS + GalaxyParameters::WORK_GROUP_SIZE - 1)
//                                 / GalaxyParameters::WORK_GROUP_SIZE;
